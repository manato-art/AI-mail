...
- **下部固定送信バー**（sticky bottom・現行フッターの中身をそのまま移設。**サイドバー幅の左padding `md:pl-16 xl:pl-60` 相当を必ず付ける** — M4で/prospectのバーがサイドバー下に潜った前例あり）: 選択N/M件 ＋ 要確認チェック（hasGenerated時のみ・**直下に説明1行**「会社ごとの内容になっていない可能性のある宛先があります。内容を確認してからチェックしてください」＝新規文言・平易）＋ 中断 ＋ 生成ボタン(N/N進捗) or 予約datetime＋送信/予約ボタン。hasGenerated による二段切替ロジックは**現行のまま**
