      ...
      <div className="space-y-5 max-w-2xl">
        ...
        <section className="rounded-xl border border-(--color-border) bg-(--color-card) overflow-hidden">
          <div className="border-b border-(--color-border) px-5 py-4">
            ...
            <p className="mt-0.5 text-xs text-(--color-muted)">テーマの表示設定を変更します</p>
          </div>
          <div className="p-5">
              ...
              {THEME_OPTIONS.map((opt) => {
                ...
                return (
                  <button
                    ...
                    className={`flex cursor-pointer flex-col items-center gap-2 rounded-xl border-2 px-4 py-4 text-sm font-medium transition-all ${
                      active
                  ...
                  >
                    <opt.Icon size={24} weight={active ? "fill" : "regular"} />
                    {opt.label}
                  </button>
                );
              })}
        ...
        <section className="rounded-xl border border-(--color-border) bg-(--color-card) overflow-hidden">
          <div className="border-b border-(--color-border) px-5 py-4">
            <h2 className="text-sm font-semibold">テーマカラー</h2>
            <p className="mt-0.5 text-xs text-(--color-muted)">アクセントカラーを選択します</p>
          </div>
          <div className="p-5">
              ...
              {ACCENT_COLORS.map((color) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      {active && <Check size={16} weight="bold" className="text-white" />}
                    </span>
                    <span className={`text-[11px] font-medium ${active ? "text-(--color-foreground)" : "text-(--color-muted)"}`}>
                      {color.label}
                    </span>
                  </button>
                );
        ...
        <section className="rounded-xl border border-(--color-border) bg-(--color-card) overflow-hidden">
          <div className="border-b border-(--color-border) px-5 py-4">
            ...
            <p className="mt-0.5 text-xs text-(--color-muted)">クイック生成時に初期選択されるサービスと人格</p>
          </div>
          <div className="space-y-3 p-5">
            <div>
              <label className="mb-1 block text-xs font-medium text-(--color-muted)">デフォルトサービス</label>
              <select
            ...
            <div>
              <label className="mb-1 block text-xs font-medium text-(--color-muted)">デフォルト人格</label>
              <select
        ...
        <section className="rounded-xl border border-(--color-danger)/30 bg-(--color-card) overflow-hidden">
          <div className="border-b border-(--color-danger)/20 px-5 py-4">
            ...
            <p className="mt-0.5 text-xs text-(--color-muted)">操作は取り消せません</p>
          </div>
          <div className="p-5">
            <button
              type="button"
              onClick={handleClearHistory}
              className="inline-flex h-10 cursor-pointer items-center gap-1.5 rounded-lg border border-(--color-danger)/30 px-4 text-sm font-medium text-(--color-danger) transition-colors hover:bg-(--color-danger-light)"
            >
              <Trash size={14} />
              生成履歴をすべて削除
            </button>
