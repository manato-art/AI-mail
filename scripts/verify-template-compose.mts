...
import type { AnalysisResult, Persona, Service, Template } from "@/lib/types";
...
const server = http.createServer((req, res) => {
  ...
  req.on("end", () => {
    // keep-alive 接続が残ると server.close 時に Windows/libuv が assert するため閉じる
...
const { composeBody, composeFromTemplate } = await import("@/lib/compose");
...
// ---------- Part 3: composeFromTemplate（generate/regenerate共通入口） ----------
const template = {
  id: 1, name: "きっかけ",
  subject: "{{company_name}}様へ「きっかけインターン」のご案内",
  body: `{{company_name}}\n{{person_name}}様\n\n${FIXED_OPEN}\n{{AI:この会社向けの一文}}\n\n${FIXED_CLOSE}`,
  compose_mode: "fixed_only", fixed_part: "", ai_brief: "", allow_attachments: 0,
  created_at: "", updated_at: "",
} as Template;
const analysisForTmpl = {
  company_name: "サンプル株式会社", representative_name: "田中太郎",
  business_summary: "x", activities: ["a"], recent_topics: [],
  compatibility: { score: "high", reason: "r" }, proposal_points: ["p"], hook: "h",
} as AnalysisResult;
const service = { id: 1, name: "採用支援", description: "d", strengths: "s", target: "t", lp_url: null, pdf_path: null, pdf_extracted_text: null, created_at: "", updated_at: "" } as Service;
const persona = { id: 1, name: "営業太郎", title: "営業", gender: "", age_range: "30代", company_name: "自社", signature_block: "sig", logic: 3, passion: 3, politeness: 3, salesiness: 3, length: 3, created_at: "", updated_at: "" } as Persona;

const fromTmpl = await composeFromTemplate(template, analysisForTmpl, service, persona);
check("composeFromTemplate: 件名の変数が置換", fromTmpl.subject.includes("サンプル株式会社") && !fromTmpl.subject.includes("{{company_name}}"));
check("composeFromTemplate: 固定文を保持", fromTmpl.body.includes(FIXED_OPEN) && fromTmpl.body.includes(FIXED_CLOSE));
check("composeFromTemplate: 会社名を置換", fromTmpl.body.includes("サンプル株式会社"));
check("composeFromTemplate: 担当者名を置換", fromTmpl.body.includes("田中太郎様"));
check("composeFromTemplate: AIゾーンを生成", fromTmpl.body.includes(MOCK_TEXT) && !fromTmpl.body.includes("{{AI:"));
