...
const OUTPUT_EXAMPLE = {
  ...
  philosophy:
    "会社理念・ミッション・ビジョン・創業の想い・大切にしている価値観（HPに記載があれば具体的に抜き出す。代表メッセージ・About・企業理念ページなどから。「ゼロから作り出す」「社会を変える」「お客様第一」等、その企業の根底にある信念。無ければnull）",
  atmosphere:
  ...
  likely_challenges:
    "この企業が事業を伸ばす上で直面しているであろう課題（事業内容・業界・規模から推測。例:「急成長フェーズでの採用ブランディング」「BtoB企業ゆえの認知度向上」等。1-2個。推測であることを自覚した上で書く）",
  empathy_point:
    "この企業の活動で共感・尊敬できる具体的なポイント（「ゼロからプロダクトを作り上げている姿勢」「地方創生に本気で取り組んでいる点」等。汎用的な褒め言葉ではなく、HPを読んだ人だけが書ける具体性で）",
  approach_strategy:
    "この企業に対する自然な提案の橋渡し。相手の事業内容や課題から、自社サービスの『どの機能・どの強み』が『相手のどの場面』で活きるかを具体的に。サービスの一般的な説明ではなく、この企業だからこそ刺さるポイントを1-2文で。",
  compatibility: {
    score: "high | medium | low",
    reason: "相性の理由",
  },
  proposal_points: [
    "この企業の具体的な状況に合わせた提案（2-3個。「SNS運用支援」のような一般論ではなく、「採用ページへの流入を増やすためのSNS施策」のようにこの企業の文脈に落とし込む）",
  ],
...
JSON出力の絶対規則（違反するとパースに失敗し、処理全体が落ちる）:
- 文字列値の中でダブルクォート " を使わないこと。日本語の引用は必ず 「」 と 『』 を使う。
  悪い例: "impressive_quote": "貴社の"日本再興"という言葉"
  良い例: "impressive_quote": "貴社の『日本再興』という言葉"
- 文字列値の中に生の改行を入れないこと。改行が必要なら \\n と書く。
- コードフェンス（\`\`\`）で囲まず、JSONだけを返すこと。
